...
func (s3s *S3Storage) Glob(dirPath string) ([]string, error) {
	bucketName, prefix, err := ParseDirPath(dirPath)
	if err != nil {
		return nil, err
	}
	...
	err = s3s.s3Client.ListObjectsV2Pages(input, func(page *s3.ListObjectsV2Output, lastPage bool) bool {
...
func (s3s *S3Storage) MkdirAll(dirPath string, perm os.FileMode) error {
	bucketName, parsePath, err := ParseDirPath(dirPath)
	// 确保路径以斜杠结尾
	if parsePath[len(parsePath)-1] != '/' {
		parsePath += "/"
	}
	...
	// 创建一个空对象来模拟目录
	s3s.s3Client.PutObject(&s3.PutObjectInput{
		Bucket: aws.String(bucketName),
...
func (s3s *S3Storage) ServeFile(w http.ResponseWriter, r *http.Request, filePath string) {
	// 获取对象
	bucketName, key, err := ParseDirPath(filePath)
...
func ParseDirPath(dirPath string) (string, string, error) {
...
func (s3s *S3Storage) OpenFile(fileName string, flag int, perm os.FileMode) (*os.File, error) {
	bucketName, key, err := ParseDirPath(fileName)
	if err != nil {
		return nil, err
	}
	if flag != os.O_RDONLY {
...
func (s3s *S3Storage) Unzip(archive, target string, currentDirectory bool) error {
	bucketName, key, err := ParseDirPath(archive)
	// 下载 S3 中的 ZIP 文件
...
func (s3s *S3Storage) CopyFileWithProgress(src, dst string, logger event.Logger) error {
	...
	bucket, key, err := ParseDirPath(dst)
	if err != err {
