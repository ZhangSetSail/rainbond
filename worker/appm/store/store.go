...
func (a *appRuntimeStore) init() error {
	...
	leaderNamespace := a.confClient.WorkerConfig.LeaderElectionNamespace
	if _, err := a.k8sClient.Clientset.CoreV1().Namespaces().Get(context.Background(), leaderNamespace, metav1.GetOptions{}); err != nil {
		if k8sErrors.IsNotFound(err) {
			_, err = a.k8sClient.Clientset.CoreV1().Namespaces().Create(context.Background(), &corev1.Namespace{
